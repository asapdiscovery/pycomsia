...
class TestContourPlotVisualizerRegression:
    ...
    def test_grid_parameter_impact_on_contours(self, contour_visualizer, test_data_dir):
        """Test that grid parameters consistently affect contour generation."""
        # Load small dataset
        data_loader = DataLoader()
        ace_file = test_data_dir / "ACE_train.sdf"
        _, mols, activities = data_loader.load_sdf_data(str(ace_file), "Activity", is_training=True)
        mols = mols[:5]  # Small subset
        activities = activities[:5]
        
        # Generate fields with two different grid resolutions
        field_calc = MolecularFieldCalculator()
        
        # Coarse grid
        grid_calc1 = MolecularGridCalculator()
        grid_spacing1, grid_dims1, grid_origin1 = grid_calc1.generate_grid(
            mols, resolution=3.0, padding=4.0
        )
        all_fields1 = field_calc.calc_field(
            mols, grid_spacing1, grid_dims1, grid_origin1
        )
        fields1 = all_fields1['train_fields']
        
        # Fine grid
        grid_calc2 = MolecularGridCalculator()
        grid_spacing2, grid_dims2, grid_origin2 = grid_calc2.generate_grid(
            mols, resolution=1.5, padding=4.0
        )
        all_fields2 = field_calc.calc_field(
            mols, grid_spacing2, grid_dims2, grid_origin2
        )
        fields2 = all_fields2['train_fields']
        
        # Generate PLS coefficients for both
        pls1 = PLSAnalysis()
        pls1.convert_fields_to_X(fields1)
        pls1.perform_loo_analysis(activities, max_components=2)
        pls1.fit_final_model(activities)
        coeffs1 = pls1.get_coefficient_fields()
        
        pls2 = PLSAnalysis()
        pls2.convert_fields_to_X(fields2)
        pls2.perform_loo_analysis(activities, max_components=2)
        pls2.fit_final_model(activities)
        coeffs2 = pls2.get_coefficient_fields()
        
        # Calculate significant ranges
        ranges1 = contour_visualizer.calculate_significant_ranges(coeffs1)
        ranges2 = contour_visualizer.calculate_significant_ranges(coeffs2)
        
        # Both should have valid ranges, but may differ due to grid resolution
        assert 'steric_field' in ranges1
        assert 'steric_field' in ranges2
        
        # Ranges should be reasonable regardless of grid resolution
        for ranges in [ranges1, ranges2]:
            steric_ranges = ranges['steric_field']
            assert steric_ranges['low'][0] < steric_ranges['low'][1] <= 0
            assert 0 <= steric_ranges['high'][0] < steric_ranges['high'][1]